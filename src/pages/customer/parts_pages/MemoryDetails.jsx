...
function MemoryDetails({ product }) {
  const navigate = useNavigate();
  const { addToCart, isInCart } = useContext(CartContext);
  const [memorySpecs, setMemorySpecs] = useState(null);
  ...
  const fetchMemorySpecs = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `http://localhost/gearsphere_api/GearSphere-BackEnd/getMemoryDetails.php?product_id=${product.product_id}`
      );

      if (response.data.success) {
        setMemorySpecs(response.data.specs);
      } else {
        setError("Failed to load memory specifications");
      }
    } catch (err) {
      setError("Failed to load memory specifications");
    } finally {
                ...
                <Table striped bordered hover responsive>
                  <tbody>
                    <tr>
                      ...
                      <td>{memorySpecs.memory_type || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Speed</strong>
                      </td>
                      <td>{memorySpecs.speed || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Modules</strong>
                      </td>
                      <td>{memorySpecs.modules || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>CAS Latency</strong>
                      </td>
                      <td>{memorySpecs.cas_latency || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Voltage</strong>
                      </td>
                      <td>{memorySpecs.voltage || "N/A"}</td>
                    </tr>
                  </tbody>
                </Table>
              </Card.Body>
