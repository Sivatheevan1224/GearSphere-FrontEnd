...
function CPUDetails({ product }) {
  const navigate = useNavigate();
  const { addToCart, isInCart } = useContext(CartContext);
  const [cpuSpecs, setCpuSpecs] = useState(null);
  ...
  const fetchCPUSpecs = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost/gearsphere_api/GearSphere-BackEnd/getCPUDetails.php?product_id=${product.product_id}`
      );

      if (response.data.success) {
        setCpuSpecs(response.data.specs);
      } else {
        setError("Failed to load CPU specifications");
      }
    } catch (err) {
      setError("Failed to load CPU specifications");
    } finally {
                ...
                <Table striped bordered hover responsive>
                  <tbody>
                    <tr>
                      ...
                      <td>{cpuSpecs.series || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Socket</strong>
                      </td>
                      <td>{cpuSpecs.socket || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Core Count</strong>
                      </td>
                      <td>{cpuSpecs.core_count || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Thread Count</strong>
                      </td>
                      <td>{cpuSpecs.thread_count || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                    ...
                    <tr>
                      ...
                      <td>
                        {cpuSpecs.core_boost_clock
                          ? `${cpuSpecs.core_boost_clock} GHz`
                          : "N/A"}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>{cpuSpecs.tdp || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Integrated Graphics</strong>
                      </td>
                      <td>{cpuSpecs.integrated_graphics ? "Yes" : "No"}</td>
                    </tr>
                  </tbody>
                </Table>
              </Card.Body>
