...
function OperatingSystemDetails({ product }) {
  const navigate = useNavigate();
  const { addToCart, isInCart } = useContext(CartContext);
  const [osSpecs, setOsSpecs] = useState(null);
  ...
  const fetchOsSpecs = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `http://localhost/gearsphere_api/GearSphere-BackEnd/getOperatingSystemDetails.php?product_id=${product.product_id}`
      );

      if (response.data.success) {
        setOsSpecs(response.data.specs);
      } else {
        setError("Failed to load Operating System specifications");
      }
    } catch (err) {
      setError("Failed to load Operating System specifications");
    } finally {
  ...
  if (loading) {
    return (
      <LoadingScreen
        message="Loading Operating System Details"
                ...
                <Table striped bordered hover responsive>
                  <tbody>
                    <tr>
                      ...
                      <td>{osSpecs.model || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Mode</strong>
                      </td>
                      <td>{osSpecs.mode || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Version</strong>
                      </td>
                      <td>{osSpecs.version || "N/A"}</td>
                    </tr>
                    <tr>
                      <td>
                        <strong>Max Supported Memory</strong>
                      </td>
                      <td>{osSpecs.max_supported_memory || "N/A"}</td>
                    </tr>
                  </tbody>
                </Table>
              </Card.Body>
